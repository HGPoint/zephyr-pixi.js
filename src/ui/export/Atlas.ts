...
function createTiledImage(img: Drawable) {
    ...
    if (context) {
        ...
        if (pattern) {
            ...
            context.fillStyle = '#000'; // Устанавливаем дефолтный цвет
            console.warn('Паттерн не создан, используется дефолтный цвет заполнения.');
        }
    ...
    pack(rect: Rect, margin: number): AtlasNode | null | undefined {
        ...
        if (this.rect.w - rect.w > this.rect.h - rect.h) {
            ...
            this.right = new AtlasNode(
                this.rect.x + rect.w + margin,
                this.rect.y,
                this.rect.w - rect.w - margin,
                this.rect.h,
            );
        } else {
            ...
            this.right = new AtlasNode(
                this.rect.x,
                this.rect.y + rect.h + margin,
                this.rect.w,
                this.rect.h - rect.h - margin,
            );
        }
...
class Atlas {
    ...
    getFrames() {
        ...
        for (const nodeName in this._terminalNodes) {
            const node = this._terminalNodes[nodeName];
