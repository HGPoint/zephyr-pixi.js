...
async function convertBlob(
    ...
    return new Promise<Blob>((resolve, reject) => {
        ...
        image.onload = function () {
            ...
            const result = dataURItoBlob(canvas.toDataURL(type));
            resolve(result);
        };
    });
}
...
export async function exportData(
    ...
    exportAs: 'export' | 'exportAll' | 'exportFigma',
) {
    ...
    return new Promise<void>(async (resolve, reject) => {
        ...
        for (const data of exportableBytes) {
            ...
            const jpegBlob = await convertBlob(blob, 'image/jpeg');
            zip.file(`${id.split(':').join('_')}${setting.suffix}${'.jpg'}`, jpegBlob, {
                base64: true,
            });
        }
