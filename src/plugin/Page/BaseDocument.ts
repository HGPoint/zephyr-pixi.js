...
export class BaseDocument implements IBaseDocument {
    ...
    private _addImage(hash: string) {
        if (!hash) return;
        const img = this._images.find((img) => {
    ...
    public async load() {
        ...
        const start = componentContentLoadQueue.length;

        while (componentContentLoad) {
            const progress = 1 - componentContentLoadQueue.length / start;
            try {
                loadProgress(
                    progress,
                    `LOADING....COMPONENTS ${componentContentLoad.componentContent.name}`,
                );
                ...
                loadProgress(
                    progress,
                    `LOADING....COMPONENTS - ERROR! ${componentContentLoad.componentContent.name}`,
                );
            }
